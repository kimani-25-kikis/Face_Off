...
const Footer = () => {
  ...
  // Define Tailwind classes for better readability
  const footerBg = "bg-gray-900"; // Dark background
  const footerText = "text-gray-300"; // Light gray text for contrast
  const goldText = "text-gold-500"; // Primary accent color

  const linkStyle = "transition-all duration-300 hover:text-gold-300 hover:underline";
  ...
  return (
    <motion.footer
      ...
      className={`${footerBg} ${footerText} py-12 lg:py-16 border-t border-gray-800 shadow-inner`}
    >
      <div className="container mx-auto px-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center md:items-start border-b border-gray-700 pb-8 mb-8 space-y-8 md:space-y-0">
          ...
          >
            <InstagramIcon 
              className={`w-10 h-10 ${goldText} transition-colors duration-300 hover:text-white hover:bg-gold-500 rounded-full p-1`} 
            />
          </motion.a>
        </div>
